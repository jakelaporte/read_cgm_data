...
class multiple_CGM(object):
    ...
    def episodes(self,name,):
        ...
        # opts = ['\<','\>']
        # gorl = st.radio(label="Choose an inequality:",
        #                       options = ['\<','\>'],
        #                       index = 0,
        #                       )
        # idx = opts.index(gorl)
        # val = st.number_input(label = "Choose the value:",
        #                       min_value = 50/const,
        #                       max_value = 250/const,
        #                       step = 10/const,
        #                       )
        # st.write(idx,val)
        # ineq = ['less','greater'][idx]
    ...
    def markov_analysis(self,name):
        ...
        int4 = st.sidebar.slider("Interval 4",
                            ...
                            value = 250,
                            step=1)
        res,fig = self.data[name].markov_chain_calculation([int1,int2,int3,int4])
        st.pyplot(fig)
        st.html(res)
    ...
    def test_develop(self,name):
        ...
        st.pyplot(fig)
        #st.pyplot(self.data[name].agp_all(False))
