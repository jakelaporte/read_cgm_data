...
class CGM(object):
    def __init__(self,
        ...
        self.stats_dict = {}
        self.first_full_day = first_full_day
        if nostats:
            pass
        else:
    ...
    @units.setter
    def units(self,val):
        # keys = ['lowest','lower','higher','highest','bins','thresh']
        keys = ['bins','thresh']
    ...
    # @property
    # def first_full_day(self):
    #     return self._first_full_day
    
    # @first_full_day.setter
    # def first_full_day(self,val):
    #     if self.first_full_day==False and val==True:
    #         self.data['stats_use']=self.data['date']!=self.days[0]
    ...
    def assign_functions(self):
        ...
        ## global parameters
        self.params['bins']= np.array([0,54,70,180,250,350])
        self.params['thresh'] = 100
    ...
    adrr_paper_overall 	adrr_paper_low 	adrr_paper_high 	adrr_easy_overall 	adrr_easy_low 	adrr_easy_high
...
1 	    25.82490 	    12.47077 	    13.35413 	        29.83252 	        14.25410 	    15.57842
    ...
    def markov_chain_calculation(self,intervals=[54,70,180,250]):
        ...
        ax.annotate("State1",xy=((intervals[0])/2-5,-0.2),
        ...
        ax.annotate("State5",xy=((intervals[3]+350)/2-5,-0.2),
                    fontsize=10,rotation = 90)
        ax.set_xlim(0,350)
        ax.set_xticks(intervals,labels=[str(i) for i in intervals])
        #st.pyplot(fig)
        tm,pi_star,er = transition_matrix(data,intervals,5,5)
        tm = pd.DataFrame(tm)
        cols = [f"x<{intervals[0]}",f"{intervals[0]}&ltx&lt{intervals[1]}",
                      f"{intervals[1]}&ltx&lt{intervals[2]}",f"{intervals[2]}&ltx&lt{intervals[3]}",
                      f"x>{intervals[3]}"]
        tm.columns=cols
        tm.index = cols
