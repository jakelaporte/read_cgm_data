...
class CGM(object):
    def __init__(self,
        ...
        self.c=1
        if units == 'mmol':
            self.series['glucose']=self.series['glucose']/18.018
            self.c=18.018
        self.periods = periods
        
        self.dt_fmt = dt_fmt
        self.units = units
        self.times = np.arange(0,1440,time_delta)
        ...
        self.set_params({'type':'paper','unit':'mg','lower':70/self.c,'conga_h':1,
    ...
    def assign_functions(self):
        ...
        ## Create a self.cohort_cols variable for cohort data in the app
        cohort_cols = []
        ...
        for key in self.stats_functions:
            ...
            if 'type' in self.stats_functions[key].keys():
                ...
                for type in types:
                    ...
                    temp = self.stats_functions[key]['f'](**self.params)
                    stats['mg'][key+'_'+type]=temp
                    if mmol != 1:
                        stats['mmol'][key+'_'+type]=temp*self.stats_functions[key]['mmol']
                    else:
                        stats['mmol'][key+'_'+type]=temp
                ...
                if mmol != 1:
                    try:
                        stats['mmol'][key]=temp*self.stats_functions[key]['mmol']
                    except:
                        stats['mmol'][key]=temp
                else:
                    stats['mmol'][key]=temp
    ...
    def all_stats(self,data,name="",units = 'mg'):
        """
        all_stats - creates a dataframe with column of name and values based on
            units. Used only to calculate the daily stats.
        """
                    ...
                    for type,calc in zip(types,calcs):
                        if calc:
                            ...
                            desc.append(self.stats_functions[key]['description']+
                                        '_'+type)
                            val = self.stats_functions[key]['f'](**self.params)
                            if units == 'mmol':
                                try:
                                    val *= self.stats_functions[key]['mmol']
                                except:
                                    pass
                            values.append(val)
                    self.params['type']=_type_
    ...
    def stats_by_day(self,units='mg'):
        if self.daily_stats is None:
            ...
            for day in self.days:
                ...
                res = pd.concat([res,self.all_stats(data,day,units)],axis=1)
    ...
    def time_in_range_report(self):
        ...
        stat7 =stats['std']
        stat8 =stats['auc_all']
        stat81 = stats['auc_wake']
        stat82 = stats['auc_sleep']
        stat9 = stats['auc_100_all+']
        stat10 = stats['auc_100_wake+']
        stat11 = stats['auc_100_sleep+']
    ...
    def poincare_plot(self,shift_minutes = 5):
        ...
        data = self.series['imputed']
        #st.write(data)
        X_shift=data.shift(-shift_minutes//self.time_delta)
    ...
    def time_series_plot(self,impute):
        plt.style.use('ggplot')
        n = len(self.periods)
        data1 = self.series['glucose']
        if n>1:
            fig,ax = plt.subplots(nrows=n,ncols=1,figsize=(8,int(n*4)))
            fig.subplots_adjust(hspace=0.25)
            for i, period in enumerate(self.periods):
                idx = np.arange(period[0],period[-1],timedelta(minutes=self.time_delta))
                ...
                if impute:
                    data2 = self.series.loc[idx]
                    data2 = data2[data2['glucose'].isna()]['imputed']
                    new_idx = data2.index
                    ax[i].scatter(new_idx,data2,s=10,color="blue",zorder=2)
                    ax[i].legend()
        else:
            ...
            if impute:
                ...
                data2 = data2[data2['glucose'].isna()]['imputed']
                new_idx = data2.index
                ax.scatter(new_idx,data2,s=10,color="blue",zorder=2)
        return(fig)
    ...
    def markov_chain_calculation(self,intervals):
        ...
        st.pyplot(fig)
        tm,pi_star = transition_matrix(data,intervals,5,5)
        tm = pd.DataFrame(tm)
